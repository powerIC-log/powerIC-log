...
핵심은 소자를 **직렬로 이어진 두 부분**으로 쪼개 보는 것이다.

- **[A] 통하는 채널**: 소스 → pinch-off 지점 (전자가 실제로 흐르는 길)
- **[B] 공핍 구간**: pinch-off 지점 → 드레인 (전자가 없는, 끊긴 구간)

전류를 정하는 건 **[A]** 다. 그런데 [A]가 보는 조건은 늘 고정이다:

- 양단 전압 = 항상 $0 \to V_{OV}$ (드레인 쪽 끝이 *"채널 전위 = $V_{OV}$"* 인 곳으로 정의되니까)
- 채널 전하 = $V_{GS}$ 가 결정 → 고정

$V_{GS}$ 도 $V_{OV}$ 도 안 변하니 **[A]가 흘리는 전류도 고정** ($=\tfrac{1}{2}kV_{OV}^2$).

그럼 늘어난 $V_{DS}$ 는 어디로? **전부 [B]가 먹는다.** [B]는 전자가 없는 공핍구간이라 저항이 거대해서, 직렬로 이은 두 부분 중 **큰 저항 쪽([B])에 여분 전압이 다 걸린다.** (예: $V_{DS}=10\text{V}$, $V_{OV}=4\text{V}$ → [A]에 4V, [B]에 6V. $V_{DS}=20\text{V}$ → [A]는 여전히 4V, [B]에 16V.)

> **강과 폭포 비유:** [A]는 강, [B]는 폭포다. 강물의 흐름은 강 자체의 경사($V_{OV}$)와 폭($V_{GS}$)이 정한다. 폭포를 아무리 높여도($V_{DS}\uparrow$) 상류의 강물이 더 빨리 흐르진 않는다 — 강은 하류 낭떠러지 높이를 신경 쓰지 않으니까. **여분의 $V_{DS}$ 는 폭포만 높일 뿐, 전류를 정하는 강([A])엔 닿지 못한다.**
