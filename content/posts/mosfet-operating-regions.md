...
> ## triode와 saturation을 가르는 기준은 이것 하나다
>
> **채널이 소스에서 드레인 끝까지 끊김 없이 이어져 있으면 triode(ohmic), 드레인에 닿기 전에 끊기면 saturation이다.**
>
> - 드레인 끝의 오버드라이브 **> 0** → 채널이 끝까지 도달 → **triode** (= $V_{DS} < V_{OV}$)
> - 드레인 끝의 오버드라이브 **≤ 0** → 드레인 앞에서 끊김 → **saturation** (= $V_{DS} \ge V_{OV}$)

**"이어져 있다"가 "두께가 균일하다"는 뜻은 아니다.** $V_{DS}$ 가 아주 작을 때($V_{DS} \ll V_{OV}$)는 채널 두께가 거의 균일해 **진짜 선형**(순수 저항)이지만, $V_{DS}$ 가 $V_{OV}$ 에 가까워질수록 드레인 쪽이 얇아져 저항이 커지고 **곡선이 휘어진다.** 그래도 **끊기지만 않으면 여전히 triode**다.

그리고 두 영역에서 **통하는 채널이 실제로 보는 전압이 다르다** — 이것이 하나는 저항처럼, 다른 하나는 전류원처럼 동작하는 이유다.

| | 통하는 채널 양단 전압 | 그래서 |
|---|---|---|
| **Triode** ($V_{DS}=1\text{V}$) | **$V_{DS}$ 전부** (1V) | $V_{DS}$ 를 올리면 미는 힘이 그대로 커짐 → **저항** |
| **경계** ($V_{DS}=V_{OV}$) | 4V | 드레인 끝 두께가 0이 되는 지점 |
| **Saturation** ($V_{DS}=6\text{V}$) | **여전히 $V_{OV}$** (4V), 나머지는 공핍이 흡수 | 채널이 보는 조건이 안 변함 → **전류 고정** |
...
### LDO는 왜 saturation에서 쓰나 — "펑펑 흘리면 좋은 것 아닌가"

스위치의 감각으로 보면 *"어차피 켤 거면 ohmic으로 완전히 켜서 $R_{DS(on)}$ 을 최소로 만드는 게 낫지 않나"* 싶다. **LDO에서는 정반대다.**

**LDO의 임무는 입력이 흔들리고 부하가 변해도 출력을 정확히 유지하는 것**이고, 그러려면 **남는 전압을 pass FET이 정확한 만큼 먹어줘야 한다.** $V_{in}=1.8\text{V}$, $V_{out}=1.2\text{V}$ 라면 누군가는 0.6V를 떨어뜨려야 한다.

| pass FET 상태 | 결과 |
|---|---|
| **Ohmic으로 완전히 켬** ($R_{DS(on)}=20\text{m}\Omega$) | 1A에서 강하가 **20mV뿐** → 출력이 1.78V가 되어 **입력을 그냥 따라감** → 레귤레이션 없음 |
| **Saturation (전류원)** | 게이트 전압으로 **정확히 0.6V를 떨어뜨리도록** 조절 → **출력 1.2V 유지** |

즉 LDO의 pass FET은 스위치가 아니라 **전자적으로 조절되는 가변저항**이고, 루프가 게이트를 실시간으로 미세조정해 강하량을 맞춘다.

**saturation이어야 하는 이유는 제어 특성에도 있다.**

- **Ohmic**: $I_D$ 가 $V_{DS}$ 에 민감 → 입력이 조금만 흔들려도 전류가 크게 변함 → 제어가 예민하고 불안정
- **Saturation**: $I_D$ 가 $V_{GS}$ 로만 정해지고 $V_{DS}$ 에 둔감 → 게이트로 깔끔하게 제어 가능 → **PSRR(입력 리플 제거)** 도 좋아짐

> **한 줄 정리:** 스위치는 **전압을 안 떨어뜨리는 것**이 목표이고(ohmic), LDO의 pass 소자는 **정확한 만큼 떨어뜨리는 것**이 목표다(saturation). 그 강하 × 전류가 그대로 열이므로 효율이 $V_{out}/V_{in}$ 로 묶이는 것도 같은 이유다.

**dropout이 바로 이 경계다.** $V_{in}$ 이 $V_{out}$ 에 너무 가까워지면 pass FET이 saturation을 유지하지 못하고 **ohmic으로 밀려 들어간다.** 그 순간 레귤레이션을 잃고 출력이 입력을 따라가기 시작하는데, 데이터시트의 **dropout voltage** 스펙이 이 지점을 말한다.
