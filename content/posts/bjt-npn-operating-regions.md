...
### 왜 saturation에서 전류가 제한되나 — BC-on은 증상이지 원인이 아니다

여기 핵심 반전이 있다. **BJT가 스스로 전류를 막는 게 아니다.** "외부 회로가 더는 못 준다"는 신호가 BC-on일 뿐이다.

1. **트랜지스터는 $\beta I_B$ 만큼 흘리려 한다.** active에선 컬렉터가 청소기처럼 이미터가 쏜 전자를 다 걷어가니까, $I_C$ 를 $\beta I_B$ 까지 밀어붙이려 한다.
2. **하지만 컬렉터가 실제로 흘릴 수 있는 최대치는 외부 회로가 정한다.** $I_C=(V_{CC}-V_{CE})/R$ 인데 $V_{CE}$ 는 $0.2\text{V}$ 밑으로 못 가므로, 물리적 상한은 $(V_{CC}-0.2)/R$.
3. **$I_B$ 를 키우다 "요구($\beta I_B$)"가 "공급 상한"을 넘어서려는 순간** → 회로가 "더는 못 줘" → $V_{CE}$ 가 $0.2\text{V}$ 바닥에 쿵 닿고 → 컬렉터가 베이스보다 낮아져 **BC 접합이 순방향(on)** 이 된다.
4. 즉 **BC-on은 "외부 회로 한계에 도달했다"는 증거**다. 이 상태에선 $I_B$ 를 더 밀어도 $I_C$ 는 상한에 묶여 안 늘고, 청소기(역방향 전기장)도 사라져 남는 전자는 베이스에 쌓여 낭비된다.

> **수도 밸브 + 가는 파이프 비유:** $I_B$ = 밸브 여는 정도(열면 물 $I_C$ 가 $\beta$배로 나오려 함), 공급 파이프 = $V_{CC}\cdot R$(초당 줄 수 있는 물량이 정해짐). **밸브를 적당히** 열면 밸브가 병목 → 물량 = 밸브가 정함 = **active** ($I_C=\beta I_B$). **너무 활짝** 열면 이제 파이프가 병목 → 밸브를 더 열어도 물량 그대로 = **saturation**. 이때 밸브 양단 압력차($V_{CE}$)는 거의 0으로 주저앉는다(꽉 열린 스위치). **제한하는 건 파이프(외부 회로)고, BC-on은 밸브가 파이프보다 활짝 열렸다는 증거일 뿐이다.**
